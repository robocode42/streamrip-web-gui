...
function toggleOutput(id) {
  const item = document.querySelector(
    `.download-item[data-download-id="${id}"]`,
  );
  if (item) {
...
async function loadFiles() {
  ...
  try {
    const url = user
      ? `/api/browse?user=${encodeURIComponent(user)}`
      : "/api/browse";
...
async function searchMusic() {
  ...
  try {
    ...
    if (data.message) {
      ...
      resultsDiv.innerHTML =
        '<div class="empty-state">NO RESULTS FOUND ON ' +
        source.toUpperCase() +
        "</div>";
      updatePaginationControls();
    }
...
document
  .getElementById("searchSource")
  .addEventListener("change", function (e) {
    const source = e.target.value;
    const albumBtn = document.querySelector(
      '.search-type-btn[onclick*="album"]',
    );
    const artistBtn = document.querySelector(
      '.search-type-btn[onclick*="artist"]',
    );
    const trackBtn = document.querySelector(
      '.search-type-btn[onclick*="track"]',
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof window.DEFAULT_SEARCH_SOURCE !== "undefined") {
    ...
    if (
      searchSource &&
      ["qobuz", "tidal", "deezer", "soundcloud"].includes(source)
    ) {
